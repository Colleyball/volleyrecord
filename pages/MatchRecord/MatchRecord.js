...
Page({
  ...
  ChoosePlayerTeamB: function (a) {
    var that = this
    var t, e = 0
    var TeamBCount = this.data.TeamBCount
    var i = this.data.TeamBNum
    var new_num = a.currentTarget.dataset.num
    var TeamBNumFlag = this.data.TeamBNumFlag
    var m = 0
    var o = -1
    var u = i.length
    for (e = 0; e < u; e++) {
    ...
    if (0 == m) {
      if (TeamBCount >= 12) {
        ...
        var new_player = [new_num, "赛事窗"]
        i.push(new_player)
        TeamBNumFlag[new_num] = 1
        TeamBCount++
      }
    } else {
      i.splice(o, 1)
      TeamBCount--
      TeamBNumFlag[new_num] = 0
    }
    
    if (TeamBCount == 0) {
      t = 1300
    }
    if (TeamBCount > 0 && TeamBCount <= 6) {
      t = 1530
    }
    if (TeamBCount > 6) {
      t = 1630
