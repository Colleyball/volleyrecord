...
Page({
  ...
  SetTeamAPlayer: function (e) {
    ...
    if (match == 3) {
      var TeamALineUp = 'TeamALineUpFourthSet'
      var TeamBLineUp = 'TeamBLineUpFourthSet'
    }
    if (match == 4) {
      var TeamALineUp = 'TeamALineUpFifthSet'
      var TeamBLineUp = 'TeamBLineUpFifthSet'
    }
  ...
  onLoad: function (options) {
    ...
    if (wx.getStorageSync('Status') == 1 || wx.getStorageSync('Status') == 2) {
      that.setData({
        ...
        ASet4: wx.getStorageSync('ASet4'),
        BSet4: wx.getStorageSync('BSet4'),
        ASet5: wx.getStorageSync('ASet5'),
        BSet5: wx.getStorageSync('BSet5'),
    ...
    if (wx.getStorageSync('Status') == 0) {
      ...
      wx.setStorageSync('ASet4', 0)
      wx.setStorageSync('BSet4', 0)
      wx.setStorageSync('ASet5', 0)
      wx.setStorageSync('BSet5', 0)
...
function EndOneSet(that, AScore, BScore) {
  ...
  if (match == 0 || match == 1 || match == 2 || match == 3) {
    if (AScore >= 25 && AScore - BScore >= 2) {
      ...
      var match_new = AMatch_new + BMatch_new

      if (match_new == 1 || match_new == 2 || (match_new == 3 && AMatch_new != 3) || (match_new == 4 && AMatch_new != 3)) {
        if (wx.getStorageSync('FirstSide') == 0) {
          var Side_New = 1
        } else {
          var Side_New = 0
        ...
        wx.setStorageSync('LastScore', Serve_New)
        wx.setStorageSync('Side', Side_New)
        EndSet()
      }
    ...
    if (BScore >= 25 && BScore - AScore >= 2) {
      ...
      if (match_new == 1 || match_new == 2 || (match_new == 3 && BMatch_new != 3) || (match_new == 4 && BMatch_new != 3)) {
        if (wx.getStorageSync('FirstSide') == 0) {
          var Side_New = 1
        } else {
          var Side_New = 0
...
function UpdateScore(that, Ascore, Bscore) {
  ...
  if (match == 3) {
    var MatchDurationSet = 'MatchDuratoinSet4'
  }
  if (match == 4) {
    var MatchDurationSet = 'MatchDuratoinSet5'
  }
...
function GetBenchPlayerTeamA(that, CourtPlayer, AllPlayer) {
  ...
  if (match == 3) {
    var SubSet = 'SubTeamASet4'
  }
  if (match == 4) {
    var SubSet = 'SubTeamASet5'
  }
...
function GetBenchPlayerTeamB(that, CourtPlayer, AllPlayer) {
  ...
  if (match == 3) {
    var SubSet = 'SubTeamBSet4'
  }
  if (match == 4) {
    var SubSet = 'SubTeamBSet5'
  }
  var SubSetInfo = wx.getStorageSync(SubSet)
  var UpPlayer = new Array()
  var OutPlayer = new Array()
